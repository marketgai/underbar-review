(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (isSorted) {
      _.each(array, function(element) {
        if (uniqNums[iterator(element)] === undefined) {
          uniqNums[iterator(element)] = element;
        }
      });
    } else {
      _.each(array, function(element) {
        if (uniqNums[element] === undefined) {
          uniqNums[element] = element;
        }
      });
    }
    return Object.values(uniqNums);
  };
  ...
  _.delay = function(func, wait) {
    var input1 = arguments[2];
    var input2 = arguments[3];
    setTimeout(function() {
      func(input1, input2);
    }
    , wait);
  ...
  _.shuffle = function(array) {
    let results = [];
    let calledIndexes = [];
    while (results.length < array.length) {
      let ind = Math.floor(Math.random() * array.length);
      if (calledIndexes.indexOf(ind) !== -1) {
        continue;
      } else {
        calledIndexes.push(ind);
